...
namespace MupadoodleAPI.Ingestion
{
    public class CSVParser : IDataParser
    {
        ...
        public List<Models.Market> parseMarkets()
        {
            CsvReader csv = new CsvReader(reader, true);
            int fieldCount = csv.FieldCount;
            List<Market> exList = new List<Market>();
            Market exObj = null;
            ...
            while (csv.ReadNextRecord())
            {
                string cityStr = null;
                double lat = 0, lng = 0;
                string theName = null, location = null;
                ...
                exObj = new Market(lat, lng, theName, cityStr, location);
                exList.Add(exObj);
            }
